# Push finkit.db snapshot to the private data repo's GitHub Release.
...
param(
    ...
    [string]$DbPath = "$PSScriptRoot\..\backend\finkit.db"
)
...
$snap = Join-Path $snapDir "finkit.db"
...
$tmpZip = Join-Path $env:TEMP "finkit_db_$stamp.zip"
...
try {
    ...
    $body = @{ tag_name = $Tag; name = "DB snapshots (rolling)"; body = "Auto-managed DB snapshots pushed by sync_db_release.bat" } | ConvertTo-Json
