...
async def get_tags(user_id: str = Depends(get_current_user_id), db: AsyncSession = Depends(get_private_db)):
...
async def create_tag(req: TagCreate, user_id: str = Depends(get_current_user_id), db: AsyncSession = Depends(get_private_db)):
...
async def update_tag(tag_id: str, req: TagUpdate, user_id: str = Depends(get_current_user_id), db: AsyncSession = Depends(get_private_db)):
...
async def delete_tag(tag_id: str, user_id: str = Depends(get_current_user_id), db: AsyncSession = Depends(get_private_db)):
    result = await db.execute(select(Tag).where(Tag.id == tag_id, Tag.user_id == user_id))
    tag = result.scalar_one_or_none()
    if not tag:
        raise HTTPException(status_code=404, detail="Tag not found")
